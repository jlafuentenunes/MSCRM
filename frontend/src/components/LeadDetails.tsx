...
import { 
    ...
    Clock, Ticket, Edit3, Loader2, User
} from 'lucide-react';
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden">
                <header className="h-16 lg:h-20 bg-white border-b border-slate-200 px-4 lg:px-12 flex items-center justify-between sticky top-0 z-30">
                    <div className="flex items-center gap-3 lg:gap-4">
                        ...
                        <div className="hidden sm:flex w-8 h-8 lg:w-10 lg:h-10 bg-slate-100 rounded-lg lg:rounded-xl items-center justify-center text-slate-400 shrink-0">
                            <User size={16} className="lg:w-5 lg:h-5" />
                        </div>
                        <div>
                            <h2 className="text-sm lg:text-xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">{lead.empresa}</h2>
                            <p className="hidden xs:block text-[8px] lg:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Gestão de Cliente / Lead</p>
                        </div>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <button form="editForm" type="submit" disabled={saving} className="bg-slate-900 text-white px-4 lg:px-8 py-2.5 lg:py-3 rounded-xl lg:rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 shadow-lg shadow-slate-200 active:scale-95 transition-all">
                            {saving ? <Loader2 className="animate-spin" size={14}/> : <Save size={16}/>} <span className="hidden sm:inline">{saving ? 'A Guardar...' : 'Guardar Alterações'}</span><span className="sm:hidden">Guardar</span>
                        </button>
                    </div>
                </header>
                            ...
                            <form id="editForm" onSubmit={handleUpdate} className="bg-white shadow-xl shadow-slate-200/40 border border-slate-100 rounded-[40px] p-8 lg:p-12 space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            <input className="flex-1 bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold text-sm focus:bg-white focus:border-blue-500 transition-all outline-none"
                                                value={lead.telemovel} onChange={e => setLead({...lead, telemovel: e.target.value})} required />
                                            <button type="button" onClick={() => window.open(`https://wa.me/${lead.telemovel.replace(/\s+/g, '')}`, '_blank')} 
                                                    className="p-4 bg-emerald-500 text-white rounded-2xl hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-100 flex items-center justify-center aspect-square">
